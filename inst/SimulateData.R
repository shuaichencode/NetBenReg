######## This program is used to generate CEdata (censored data) and CEdata_uncensor (full complete data) ####
...
data1=data.frame(id=1:n,survival=data$Follow.up,dead=data$Death,Trt=data$Trt,data$Covariate,
...
CEdata=data1 %>% mutate(across(c(Age65,LBBB,Female), as.factor))	#set categorical variables as factor
CEdata[8:23]=CEdata[8:23]/1000		#set cost in $1000
save(CEdata,file = 'CEdata.RData')

# save full uncensored data for comparison
data.uncensor=data.frame(id=1:n,survival=data$TL,dead=rep(1,n),Trt=data$Trt,data$Covariate,
	cost=data$Cost_true,tot.cost=apply(data$Cost_true,1,sum),
	QALY=data$QALY_true,tot.QALY=apply(data$QALY_true,1,sum)
	)
CEdata_uncensor=data.uncensor %>% mutate(across(c(Age65,LBBB,Female), as.factor))	
CEdata_uncensor[8:23]=CEdata_uncensor[8:23]/1000		#set cost in $1000
save(CEdata_uncensor,file = 'CEdata_uncensor.RData')

####  Use potential outcomes (counterfactuals) to calculate ICER and INBs to evaluate methods ##########
lambda=seq(0,6,3)
...
#calculate causal average INB within 10-year ("Using true outcomes" method), the last row is for WTP=NA (Effect only) and thus is mean extra effect instead of INB
