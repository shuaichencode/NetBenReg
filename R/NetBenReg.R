#' NetBenReg: Net-benefit Regression for Possibly Censored Cost-effectiveness Data 
#'
#' @section NetBenReg functions:
#' NetBenReg, plot.NetBenReg, plot.NetBenRegDR
#'
#' @docType package
#' @name NetBenReg
NULL
...
# Note:
# The NetBenReg allows factor covariates Z (but need to set as factor before using NetBenReg)
...
#'
#' This function estimates the survival probability using Kaplan-Meier method, used to calculate weights of inverse probability of censoring.
...
NetBenReg.CC<-function(X,delta,Cost.total,Eff.total,group,Z=NULL,interaction=NULL,lambda=NULL,Cost.only=FALSE)
{
  lambda1=lambda
  ...
  fit.gee<-geepack::geeglm(as.formula(form),id=id,data=data)
...
NetBenReg.AL<-function(X,delta,Cost.total,Eff.total,group,Z=NULL,interaction=NULL,lambda=NULL,Cost.only=FALSE)
{
  lambda1=lambda
  if(is.null(lambda))lambda1=1
  if(Cost.only)lambda1=0
...
#'
#' This function uses simple weighted method for net-benefit regression.
#' @param X a vector containing continuous positive follow-up time.
#' @param delta a vector containing binary indicator of event (such as death), 1 - complete, 0 - censored.
#' @param Cost.total a vector containing observed total costs.
#' @param Eff.total a vector containing observed total effectiveness. If not provided, assume effectiveness is survival. 
...
NetBenReg.SW<-function(X,delta,Cost.total,Eff.total,group,Z=NULL,interaction=NULL,Sep.K=TRUE,lambda=NULL,Cost.only=FALSE)
{
  lambda1=lambda
  if(is.null(lambda))lambda1=1
  if(Cost.only)lambda1=0
...
NetBenReg.PT<-function(X,delta,Cost.grp,Eff.grp,Part.times,group,Z=NULL,interaction=NULL,Sep.K=TRUE,lambda=NULL,Cost.only=FALSE)
{
  lambda1=lambda
  if(is.null(lambda))lambda1=1
  if(Cost.only)lambda1=0
  ...
  x=w0=d=totalQ=DV=matrix(0,dim(data)[1],length(Part.times)-1)
  K=numeric(dim(data)[1])
...
#'
#' This function uses doubly robust simple weighted method for net-benefit regression.
#' @param X a vector containing continuous positive follow-up time.
#' @param delta a vector containing binary indicator of event (such as death), 1 - complete, 0 - censored.
#' @param Cost.total a vector containing observed total costs.
#' @param Eff.total a vector containing observed total effectiveness. If not provided, assume effectiveness is survival. 
...
#' @param interaction a vector containing covariate names to be included in interactions with treatment, must be a subset of variable names in Z, otherwise the variable not in Z will be ignored. Defaults to NULL.
#' @param Sep.K logical, if TRUE, estimate K (survival function of censoring time, used in inverse probability of censoring weighting) using Kaplan-Meier estimator within each group separately. Defaults to TRUE.
#' @param PS.trim a value between (0, 0.5) to trim extreme propensity scores outside the range of (PS.trim, 1-PS.trim). Defaults to 0.05.
#' @param lambda a vector or scalar containing cost-effectiveness threshold values (e.g., willingness-to-pay for 1 unit of additional effectiveness).
#' @param DR.Reg.SE logical, if TRUE, report SE (and p-value) for the regression part of doubly robust method, otherwise only coefficient is reported. Defaults to TRUE.
#' @param Cost.only logical. If TRUE, fit a regression with dependent variable as cost. Defaults to FALSE.
#' @keywords net-benefit regression, simple weighted, doubly robust

NetBenReg.SW.DR<-function(X,delta,Cost.total,Eff.total,group,Z=NULL,PS.Z=NULL,interaction=NULL,Sep.K=TRUE,PS.trim=0.05,lambda=NULL,DR.Reg.SE=TRUE,Cost.only=FALSE)
{
  lambda1=lambda
  if(is.null(lambda))lambda1=1
  if(Cost.only)lambda1=0
...
NetBenReg.PT.DR<-function(X,delta,Cost.grp,Eff.grp,Part.times,group,Z=NULL,PS.Z=NULL,interaction=NULL,Sep.K=TRUE,PS.trim=0.05,lambda=NULL,DR.Reg.SE=TRUE,Cost.only=FALSE)
{
  lambda1=lambda
  if(is.null(lambda))lambda1=1
  if(Cost.only)lambda1=0
...
coef.split=coef.split2=matrix(0,length(Part.times)-1,dim(covar)[2])
Ksi=array(0,c(length(Part.times)-1,dim(data)[1],dim(covar)[2]))
B.split=array(0,c(length(Part.times)-1,length(Part.times)-1,dim(covar)[2],dim(covar)[2]))
x=w0=w2=d=K.new=m1=m0=totalQ=DV=matrix(0,dim(data)[1],length(Part.times)-1)
mu1=mu0=est.IPW=numeric(length(Part.times)-1)
...
#' @param Part.times a vector of length m, containing end time points (in days) of each time interval. Must be monotonically increasing (eg, 1st interval is Day 1 to Part.times[1], 2nd is interval Day Part.times[1]+1 to Part.times[2]). Required if using PT method without Eff provided, or truncating grouped costs and effectiveness if they may be partially outside time limit L. 
...
fit1<-nbreg(Followup=data$survival,		#follow-up time	
	...
	lambda=lambda,				#cost-effectiveness threshold value
...
#  if(!is.null(Eff))if(!is.numeric(Eff)) stop("Eff must be numeric.\n")
  ...
  if(!is.null(Part.times)){
	Part.times=unlist(Part.times)
	if(min(Part.times)<0) stop("Values in Part.times cannot be negative.\n")
	if(!is.null(Cost)){
	  if(length(Part.times)!=m){ 
		if(m>1)stop("Length of Part.times is different to number of columns of Cost.\n") else {
			Part.times=NULL; warning("Part.times is not used since only total Cost is provided.\n")}
	  }
	}else m=length(Part.times)
  ...
  } else
  {
	...
	if(dim(Eff)[2]!=m) stop("Length of Part.times is different to number of columns of Eff.\n")
  }
...
#  if((is.null(Part.times))&(!is.null(Eff))) cat("Note: Part.times is not provided. Assume all costs and effectiveness are evenly spreaded until follow-up time when truncated by time limit L.\n")
#  if((is.null(Part.times))&(is.null(Eff))) cat("Note: Part.times is not provided. Assume all costs are evenly spreaded until follow-up time when truncated by time limit L.\n")
  ...
  }else
  {
  	for(i in 1:n)
	{
  		for(j in 1:m)
		{
			...
			{
				if (FollowupL[i]<=Part.times[j]) {Eff[i,j]=0;if(!is.null(Cost))Cost[i,j]=0}
				else {	#prorate
				  Eff[i,j]=Eff[i,j]*(FollowupL[i]-Part.times[j])/(min(Followup[i],Part.times[j+1])-Part.times[j])	
				  if(!is.null(Cost))Cost[i,j]=Cost[i,j]*(FollowupL[i]-Part.times[j])/(min(Followup[i],Part.times[j+1])-Part.times[j])	
				}
			}
		}  
  	}
  ...
  if(Method=="CC"){
	...
	{
		fit=NetBenReg.CC(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,
			interaction=interaction,lambda=lambda[j])
		results[[j]]<-fit
  	}
    }
    if(Eff.only){
	fit=NetBenReg.CC(X=FollowupL,delta=deltaL,Cost.total=rep(0,n),Eff.total=Eff.total,group=group,Z=Z,interaction=interaction)
	results[[length(lambda)+1]]<-fit
    }
    if(Cost.only){
	fit=NetBenReg.CC(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,interaction=interaction,Cost.only=TRUE)
	results[[n.lam]]<-fit
    }
  }else
  if(Method=="AL"){
	...
	for(j in 1:length(lambda))
	{
		fit=NetBenReg.AL(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,
			interaction=interaction,lambda=lambda[j])
		results[[j]]<-fit
  	}
    }
    if(Eff.only){
	fit=NetBenReg.AL(X=FollowupL,delta=deltaL,Cost.total=rep(0,n),Eff.total=Eff.total,group=group,Z=Z,interaction=interaction)
	results[[n.lam]]<-fit
    }
    if(Cost.only){
	fit=NetBenReg.AL(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,interaction=interaction,Cost.only=TRUE)
	results[[n.lam]]<-fit
    }
  }else
  if(Method=="SW"){
	...
	if(!is.null(lambda)){
		for(j in 1:length(lambda))
		{
			fit=NetBenReg.SW(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,
				interaction=interaction,lambda=lambda[j],Sep.K=Sep.K)
			results[[j]]<-fit
  		}
    	}
    	if(Eff.only){
		fit=NetBenReg.SW(X=FollowupL,delta=deltaL,Cost.total=rep(0,n),Eff.total=Eff.total,group=group,Z=Z,interaction=interaction,Sep.K=Sep.K)
		results[[n.lam]]<-fit
    	}
      if(Cost.only){
		fit=NetBenReg.SW(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,interaction=interaction,Sep.K=Sep.K,Cost.only=TRUE)
		results[[n.lam]]<-fit
      }
    }else{
	...
	if(!is.null(lambda)){
		for(j in 1:length(lambda))
		{
			fit=NetBenReg.SW.DR(X=FollowupL,delta=deltaL,Cost.total=Cost.total,Eff.total=Eff.total,group=group,Z=Z,
				PS.Z=PS.Z,interaction=interaction,lambda=lambda[j],Sep.K=Sep.K,PS.trim=PS.trim,DR.Reg.SE=DR.Reg.SE)
			results[[j]]<-fit
  		}
    	}
    	if(Eff.only){
		fit=NetBenReg.SW.DR(X=FollowupL,delta=deltaL,Cost.total=rep(0,n),Eff.total=Eff.total,group=group,Z=Z,
  ...
  if(Method=="PT"){
	...
	if(!is.null(lambda)){
		for(j in 1:length(lambda))
		{
			fit=NetBenReg.PT(X=FollowupL,delta=deltaL,Cost.grp=Cost,Eff.grp=Eff,Part.times=Part.times,group=group,Z=Z,
				interaction=interaction,lambda=lambda[j],Sep.K=Sep.K)
			results[[j]]<-fit
  		}
    	}
    	if(Eff.only){
		fit=NetBenReg.PT(X=FollowupL,delta=deltaL,Cost.grp=matrix(0,n,m),Eff.grp=Eff,Part.times=Part.times,group=group,Z=Z,
	...
	if(!is.null(lambda)){
		for(j in 1:length(lambda))
		{
			fit=NetBenReg.PT.DR(X=FollowupL,delta=deltaL,Cost.grp=Cost,Eff.grp=Eff,Part.times=Part.times,group=group,Z=Z,
				PS.Z=PS.Z,interaction=interaction,lambda=lambda[j],Sep.K=Sep.K,PS.trim=PS.trim,DR.Reg.SE=DR.Reg.SE)
			results[[j]]<-fit
  		}
    	}
    	if(Eff.only){
		fit=NetBenReg.PT.DR(X=FollowupL,delta=deltaL,Cost.grp=matrix(0,n,m),Eff.grp=Eff,Part.times=Part.times,group=group,Z=Z,
			PS.Z=PS.Z,interaction=interaction,Sep.K=Sep.K,PS.trim=PS.trim,DR.Reg.SE=DR.Reg.SE)
		results[[n.lam]]<-fit
    	}
      if(Cost.only){
  ...
  if(!Doubly.Robust) class(results) <- "NetBenReg" else class(results) <- "NetBenRegDR" 
  return(results)
}
...
#' @param ... other graphical parameters for plot().
...
plot.NetBenRegDR<-function(object, add=FALSE,xlab = "WTP",ylab="Probability Treatment 1 is cost-effective",pch=20,cex=1,lty=1,lwd=1,type='o', ...)
{
...
plot.NetBenReg<-function(object, subgroup=NULL, add=FALSE,xlab = "WTP",ylab="Probability Treatment 1 is cost-effective",pch=20,cex=1,lty=1,lwd=1,type='o', ...)
